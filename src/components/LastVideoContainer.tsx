...
export default function LastVideoContainer({ videoData }: LastVideoContainerProps) {
  ...
  const videoAgeHours = videoData?.published_at 
    ? (Date.now() - new Date(videoData.published_at).getTime()) / (1000 * 60 * 60)
    : Infinity;
  
  // Hide stats if video is less than 1 hour old (YouTube API returns inaccurate data)
  const showStats = videoAgeHours >= 1;
        ...
        <div className="flex flex-col gap-[15px] grow items-start justify-start min-h-px min-w-px relative shrink-0" data-name="Video Info Container" data-node-id="57:7">
          ...
          {showStats ? (
            ...
            <div className="text-sm text-gray-600 italic">
              Stats will be available 1 hour after upload (YouTube API delay)
            </div>
          )}
        </div>
      </div>
